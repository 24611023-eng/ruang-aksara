...
<head>
    ...
    <style>
        ...
        /* Header hijau */
        .navbar {
            background: linear-gradient(135deg, #255335 0%, #183923 100%) !important;
        ...
        .notif-panel .items-center {
            align-items: center;
        }
        
        .notif-panel .justify-between {
        ...
        .notif-panel .border-b {
            border-bottom-width: 1px;
        }
        
        .notif-panel .border-gray-100 {
            border-color: #f3f4f6;
        }
        
        .notif-panel .bg-gradient-to-r {
            background-image: linear-gradient(to right, #f0fdf4, #ffffff);
        }
        
        .notif-panel .text-gray-800 {
            color: #1f2937;
        }
        
        .notif-panel .text-gray-500 {
            color: #6b7280;
        }
        
        .notif-panel .text-green-700 {
            color: #15803d !important;
            font-weight: 700 !important;
        }
        
        .notif-panel .hover\:text-green-800:hover {
        ...
        .notif-panel .text-center {
            text-align: center;
        }
        
        .notif-panel .max-h-80 {
            max-height: 20rem;
        }
        
        .notif-panel .overflow-y-auto {
            overflow-y: auto;
        }
        
        .notif-panel .divide-y > * + * {
...
<body class="font-sans antialiased">
    ...
    <div class="main-wrapper" id="mainWrapper">
        <div class="content-wrapper">
            <!-- Navbar -->
            <nav class="navbar" style="margin: 0.9rem auto 0; padding: 0 18px; width: 100%; box-sizing: border-box;">
                <div class="container mx-auto px-0 py-3">
                    <div class="flex justify-between items-center gap-4">
                        <a href="/" class="text-2xl font-bold text-white flex items-center gap-2 nav-brand" style="text-decoration: none;">
                            <i class="fas fa-book" style="color: #a3e635;"></i> Ruang Aksara
                        </a>
                        <div class="flex gap-3 items-center">
                            <a href="{{ route('cart.index') }}" class="cart-badge" style="position: relative; background: #a3e635; color: #1e3e2a; font-weight: 700; border-radius: 0.9rem; padding: 0.65rem 1.4rem; transition: all 0.25s ease; box-shadow: 0 6px 14px rgba(163,230,53,0.28); text-decoration: none; display: inline-flex; align-items: center; gap: 0.55rem; font-size: 1rem;">
                                <i class="fas fa-shopping-cart"></i> Keranjang
                                <span id="globalCartCount" style="position: absolute; top: -6px; right: -8px; background: #dc2626; color: white; border-radius: 999px; min-width: 18px; height: 18px; padding: 0 5px; display: inline-flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 800; border: 2px solid #183923; box-shadow: 0 4px 10px rgba(220,38,38,0.35);">0</span>
                            </a>
                            @auth
                                <div style="position: relative;">
                                    <button type="button" id="notifBtn" style="color: #d9f99d; border: 2px solid #a3e635; border-radius: 0.95rem; padding: 0.6rem 1.35rem; font-weight: 700; display: inline-flex; align-items: center; gap: 0.55rem; transition: all 0.25s ease; background: transparent; cursor: pointer; position: relative;">
                                        <i class="fas fa-bell"></i> Notifikasi
                                        @php
                                            $unreadCount = auth()->check() && auth()->user()->unreadNotifications ? auth()->user()->unreadNotifications->count() : 0;
                                        @endphp
                                        @if($unreadCount > 0)
                                            <span id="notifBadge" style="position: absolute; top: -4px; right: -4px; background: #dc2626; color: white; border-radius: 50%; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: bold;">{{ $unreadCount }}</span>
                                        @endif
                                    </button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="mb-6">
                            ...
                            <p class="text-gray-700 text-justify" style="line-height:1.5">{{ $book->deskripsi }}</p>
                        </div>
