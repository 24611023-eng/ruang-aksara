                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0;">
                        <a href="/" class="nav-brand">
                            @if(file_exists(public_path('images/ruang-aksara-logo-fix.jpeg')))
                                <img src="{{ asset('images/ruang-aksara-logo-fix.jpeg') }}" alt="Ruang Aksara" style="width:28px;height:28px;object-fit:contain;">
                            @else
                        ...
                        <div style="display: flex; gap: 0.75rem; align-items: center;">
                            @auth
                                ...
                                <div style="position: relative;">
                                    <button type="button" id="notifBtn" class="btn" style="background: transparent; color: #a3e635; border: 1.5px solid #a3e635; font-weight: 600; border-radius: 0.75rem; padding: 0.5rem 0.875rem; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 0.5rem; font-size: 0.95rem; position: relative; cursor: pointer;" onmouseover="this.style.backgroundColor='rgba(163, 230, 53, 0.1)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.backgroundColor='transparent'; this.style.transform='translateY(0)';">
...
@endif
                        ...
                        <div class="book-info">
                            ...
                            @php
                                $currentStock = $book->stok ?? 0;
                            @endphp
                            <div class="stock-indicator" aria-label="Stok {{ $book->judul }}">
                                <i class="fas fa-box"></i>
                                @if($currentStock > 0)
                                    <span>{{ $currentStock }} tersedia</span>
                                    <span class="stock-tag {{ ($currentStock <= ($alertThreshold ?? 5)) ? 'low' : '' }}">
                                        {{ ($currentStock <= ($alertThreshold ?? 5)) ? 'Stok menipis' : 'Stok aman' }}
                                    </span>
                                @else
                                    <span class="stock-count-empty">Stok habis</span>
                                    <span class="stock-tag empty">Butuh restock</span>
                                @endif
                            </div>
    ...
    <script>
        ...
        @include('partials.welcome-footer', ['hasSidebar' => auth()->check() && auth()->user()->role === 'user', 'footerOffsetScale' => 0.25])
</body>
</html>