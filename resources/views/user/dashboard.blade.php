...
<head>
    ...
    <!-- Hard fallback: paksa sidebar selalu terlihat untuk user -->
    <style>
    ...
    <div class="main-wrapper sidebar-open" id="mainWrapper">
        <div class="content-wrapper">
            <div class="mx-auto max-w-6xl space-y-8">
                ...
                <section class="rounded-3xl bg-linear-gradient-katalog p-8 text-white shadow-xl" style="background: linear-gradient(135deg, rgba(45, 90, 61, 0.15) 0%, rgba(30, 62, 42, 0.1) 100%); border: 2px solid rgba(163, 230, 53, 0.2); backdrop-filter: blur(10px);">
                    <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                        <div class="space-y-3">
                            <h2 class="text-4xl font-bold" style="color: #a3e635;">Dashboard</h2>
                            <p class="text-lg" style="color: #ffffff; text-shadow: 0 1px 4px rgba(0, 0, 0, 0.3);">Kelola aktivitas dan jelajahi koleksi buku terbaik kami</p>
                        </div>
                                    ...
                                    <div class="book-card" style="--book-scale: {{ $isTop3 ? '1.02' : '1' }}; background-image: linear-gradient(rgba(255,255,255,0.04), rgba(255,255,255,0.04)), url('{{ $cover }}'); background-size: cover; background-position: center;">
                                        ...
                                        <div style="backdrop-filter: blur(6px); background: linear-gradient(180deg, rgba(0,0,0,0.05), rgba(255,255,255,0.6));">
                                            ...
                                            <div class="space-y-3 bg-white/95 p-5">
                                                ...
                                                @endphp
                                                <div class="flex items-center gap-2 text-sm font-semibold" aria-label="Stok {{ $book->judul }}">
                                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full
                                                        {{ $currentStock === 0 ? 'bg-red-100 text-red-700' : (($currentStock <= ($alertThreshold ?? 5)) ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-700') }}">
                                                        <i class="fas fa-box"></i>
                                                        @if($currentStock === 0)
                                                            Stok habis
                                                        @elseif($currentStock <= ($alertThreshold ?? 5))
                                                            {{ $currentStock }} tersisa
                                                        @else
                                                            {{ $currentStock }} tersedia
                                                        @endif
                                                    </span>
                                                    @if($currentStock <= ($alertThreshold ?? 5))
                                                        <span class="text-xs text-red-600 font-medium">{{ $currentStock === 0 ? 'Segera restock' : 'Segera dibeli sebelum habis' }}</span>
                                                    @endif
                                                </div>
