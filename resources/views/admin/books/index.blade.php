...
@section('title', 'Kelola Buku - ' . ucfirst(auth()->user()->role))
...
<div class="w-full py-6" style="margin: 0 !important; padding: 0 1rem 0 0 !important;">
    ...
    <div class="text-white rounded-2xl p-8 mb-6 border-2" style="background: rgba(45, 90, 61, 0.4); backdrop-filter: blur(15px); border-color: rgba(163, 230, 53, 0.3);">
        <div class="flex flex-row gap-8 items-center">
            <!-- Title Section -->
            <div class="flex-1">
                <h1 class="text-5xl font-bold" style="color: #a3e635;">📚 Kelola Buku</h1>
                <p class="text-white/90 text-base mt-2">Jelajahi koleksi buku terbaik kami untuk memperkaya wawasan dan pengetahuan Anda</p>
            </div>
            <!-- Button Section -->
            <div class="flex-shrink-0">
                <a href="{{ route('admin.books.create') }}" class="inline-block text-white px-6 py-3 rounded-lg transition font-medium" style="background: #a3e635; color: #1e3e2a;">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 gap-4">
                ...
                @php
                    ...
                    $stockBadgeLabel = 'Stok aman';
                    if ($currentStock === 0) {
                        $stockBadgeClasses = 'bg-red-100 text-red-700';
                        $stockBadgeLabel = 'Stok habis';
                    } elseif ($currentStock <= ($alertThreshold ?? 5)) {
                        $stockBadgeClasses = 'bg-yellow-100 text-yellow-700';
                        $stockBadgeLabel = 'Stok menipis';
                    }
                @endphp
                            ...
                            <div class="grid grid-cols-3 gap-4 h-full">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <div class="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
                                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full {{ $stockBadgeClasses }}">
                                            <i class="fas fa-box"></i>
                                            {{ $stockBadgeLabel }} ({{ $currentStock }})
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="col-span-1">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="flex items-center">
                                            <span class="text-lg font-bold">{{ $currentStock }}</span>
                                            @if($currentStock == 0)
                ...
                <div class="mt-6 flex justify-between items-center bg-white p-4 rounded-lg shadow-md">
                    ...
                    @else
                        <a href="{{ $books->previousPageUrl() }}" 
                           class="px-4 py-2 text-white rounded hover:opacity-90 transition flex items-center" style="background: #a3e635; color: #1e3e2a;">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                            </svg>
                            Previous
