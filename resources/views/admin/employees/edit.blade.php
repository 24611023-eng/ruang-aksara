...
<div class="w-full py-6" style="margin: 0 !important; padding: 0 1rem 0 0 !important;">
    <div class="text-white rounded-2xl p-8 mb-6 border-2" style="background: rgba(45, 90, 61, 0.4); backdrop-filter: blur(15px); border-color: rgba(163, 230, 53, 0.3);">
        <div class="flex flex-row gap-8 items-center">
            <div class="flex-1">
                <h1 class="text-5xl font-bold" style="color: #a3e635;">✏️ Edit Pegawai</h1>
                <p class="text-white/90 text-base mt-2">Perbarui informasi pegawai (nama &amp; NIK dapat diubah oleh owner/admin).</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.employees.update', $employee) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" value="{{ old('name', $employee->name) }}" class="w-full border p-2 rounded" required>
            </div>
            <div class="mb-3">
                <label class="block text-sm font-medium">Email</label>
                <input type="email" name="email" value="{{ old('email', $employee->email) }}" class="w-full border p-2 rounded" required>
            </div>
            <div class="mb-3">
                <label class="block text-sm font-medium">Posisi / Hak</label>
                <input type="text" name="position" value="{{ old('position', $employee->position) }}" class="w-full border p-2 rounded" placeholder="contoh: kasir, logistik, admin stok">
            </div>
            <div class="mb-3">
                <label class="block text-sm font-medium">Telepon</label>
                <input type="text" name="telepon" value="{{ old('telepon', $employee->telepon) }}" class="w-full border p-2 rounded">
            </div>

            <div class="mb-3">
                <label class="block text-sm font-medium">NIK</label>
                <input type="text" name="nik" value="{{ old('nik', $employee->nik ?? '') }}" inputmode="numeric" maxlength="16" pattern="^[0-9]{16}$" title="NIK harus berisi 16 angka" class="w-full border p-2 rounded">
            </div>
