...
class LoanStockController extends Controller
{
    ...
    /**
     * Show edit form for a loan book (details & cover)
     */
    public function edit(LoanBook $loanBook)
    {
        ...
        return view('admin.loan-stock.edit', compact('loanBook', 'categories'));
    }

    /**
     * Update book details (judul, penulis, image, etc.)
     */
    public function updateDetails(Request $request, LoanBook $loanBook)
    {
        $validated = $request->validate([
            'judul' => 'required|string|max:255',
            ...
            'penerbit' => 'nullable|string|max:255',
            'isbn' => 'nullable|string|max:50|unique:loan_books,isbn,' . $loanBook->id,
            'halaman' => 'nullable|integer|min:1',
            'deskripsi' => 'nullable|string|max:1000',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
        ...
        try {
            ...
            $loanBook->update(array_filter([
                'judul' => $validated['judul'] ?? $loanBook->judul,
                'penulis' => $validated['penulis'] ?? $loanBook->penulis,
                'kategori' => $validated['kategori'] ?? $loanBook->kategori,
                'penerbit' => $validated['penerbit'] ?? $loanBook->penerbit,
                'isbn' => $validated['isbn'] ?? $loanBook->isbn,
                'halaman' => $validated['halaman'] ?? $loanBook->halaman,
                'deskripsi' => $validated['deskripsi'] ?? $loanBook->deskripsi,
                'image' => $validated['image'] ?? $loanBook->image,
            ]));

            return redirect()->route('admin.loan-stock.index')->with('success', 'Detail buku berhasil diperbarui');
        } catch (\Exception $e) {
            return redirect()->back()->withErrors(['error' => 'Gagal memperbarui detail buku: ' . $e->getMessage()])->withInput();
        }
    }
