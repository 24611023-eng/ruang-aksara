...
class CartController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        $rules = [
            ...
            'telepon' => ['required', 'string', 'max:20', 'regex:/^(\+62|62|0)[0-9]{9,13}$/'],
        ...
        $isCodPayment = $request->payment_method === 'cash';
        $shouldAutoVerifyCod = $isCodPayment && $this->isAutoVerifyCodEnabled();
        ...
        $order = \App\Models\Order::create([
            ...
            'payment_status' => $shouldAutoVerifyCod ? 'verified' : 'pending',
        ]);
        ...
        // If COD auto verification is enabled, advance order immediately
        if ($shouldAutoVerifyCod) {
            try {
        ...
        foreach ($orderItems as $bookId => $itemData) {
            $book = $itemData['book'];
            // PENTING: Hanya kurangi stok ketika COD auto-verifikasi aktif, transfer orders menunggu verifikasi
            if ($shouldAutoVerifyCod) {
    ...
    private function isAutoVerifyCodEnabled(): bool
    {
        try {
            $settings = Cache::get('payment_verification_settings');
        } catch (\Throwable $e) {
            $settings = null;
        }

        if (!is_array($settings)) {
            $settings = session('payment_verification_settings', []);
        }
        ...
        return (bool) ($settings['auto_verify_cod'] ?? true);
    }
