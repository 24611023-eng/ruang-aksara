...
Route::post('/login', function (Request $request) {
        ...
        // If normal user login failed, try employee guard so pegawai can login using same form
        if (Auth::guard('employee')->attempt($credentials)) {
            $request->session()->regenerate();
            $employee = Auth::guard('employee')->user();
            \Log::info('Employee login successful - Employee ID: ' . $employee->id . ', Email: ' . $employee->email);
...
// These endpoints are intentionally public so that the controller can
// decide whether the caller is an authenticated `user` or `employee`.
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::patch('/profile', function (Request $request) {
        ...
        $nikChanged = $isUserRole ? false : $request->input('nik') !== ($user->nik ?? '');
        ...
        if (!$isUserRole) {
            $validationRules['nik'] = 'nullable|string|max:50';
        }

        $request->validate($validationRules);

        // If any change, require password verification
        if ($nameChanged || $alamatChanged || $teleponChanged || $tanggalLahirChanged || $nikChanged) {
            $request->validate([
        ...
        // Only update allowed fields (NEVER email)
        $updateFields = ['name', 'alamat', 'telepon', 'tanggal_lahir'];
        if (!$isUserRole) {
            $updateFields[] = 'nik';
        }
    ...
    Route::post('/profile/foto', function (Request $request) {
        try {
            ...
            if ($request->hasFile('foto_profil')) {
                ...
                // If the user is owner, also set the `avatar` column to the public URL
                ...
                \Log::info('Profile foto uploaded: user_id=' . ($user->id ?? 'unknown') . ', path=' . $path . ', url=' . $publicUrl . ', avatar_set=' . (isset($user->role) && $user->role === 'owner' ? 'yes' : 'no'));
...
Route::middleware(['auth', 'role:admin,owner'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('/dashboard', function () {
        ...
        // Use alert threshold from notification settings (cache/session) if present
        try {
            $alertThreshold = \Illuminate\Support\Facades\Cache::get('notification_settings.alert_threshold')
                ?? \Illuminate\Support\Facades\Cache::get('notification_settings', [])["alert_threshold"]
                ?? session('notification_settings.alert_threshold')
                ?? 5;
        } catch (\Exception $e) {
            $alertThreshold = 5;
        }
        ...
        try {
            $lowStockBooks = Book::where('stok', '<=', $alertThreshold)
                                ->orderBy('stok', 'asc')
                                ->get();
        } catch (\Exception $e) {
        ...
        try {
            $lowLoanStockBooks = LoanBook::where('loan_stok', '<=', $alertThreshold)
        ...
        return view('admin.dashboard', compact('stats', 'recentOrders', 'books', 'lowStockBooks', 'lowLoanStockBooks', 'alertThreshold'));
    })->name('dashboard');
    ...
    Route::get('/operational-costs/export', [OperationalCostController::class, 'export'])->name('operational-costs.export');
    ...
    // Owner-only: Kelola Pegawai dan haknya
    Route::middleware('role:owner')->prefix('employees')->name('employees.')->group(function () {
        Route::get('/', [\App\Http\Controllers\OwnerEmployeeController::class, 'index'])->name('index');
        Route::get('/create', [\App\Http\Controllers\OwnerEmployeeController::class, 'create'])->name('create');
        Route::post('/', [\App\Http\Controllers\OwnerEmployeeController::class, 'store'])->name('store');
        Route::get('/{employee}/edit', [\App\Http\Controllers\OwnerEmployeeController::class, 'edit'])->name('edit');
        Route::patch('/{employee}', [\App\Http\Controllers\OwnerEmployeeController::class, 'update'])->name('update');
        Route::delete('/{employee}', [\App\Http\Controllers\OwnerEmployeeController::class, 'destroy'])->name('destroy');
    });
    ...
    Route::prefix('settings')->name('settings.')->group(function () {
        ...
        // Removed clear-cache and backup-database routes per request
    ...
    // ✅ ADMIN: Users list + search (Admin/Owner)
    ...
    Route::prefix('loans')->name('loans.')->group(function () {
        ...
        Route::get('/{loan}', function (Loan $loan) {
            try {
                ...
                // Load book relation if book_id exists
                if ($loan->book_id) {
                    $loan->load('book');
                }
                
                // Load loanBook relation if loan_book_id exists
                if ($loan->loan_book_id) {
                    $loan->load('loanBook');
                }
                
                return response()->json($loan);
            } catch (\Exception $e) {
                \Log::error('Error loading loan: ' . $e->getMessage());
                return response()->json(['error' => 'Failed to load loan details'], 500);
            }
        })->name('show');
        ...
        Route::patch('/{loan}/return', function (Request $request, Loan $loan) {
            ...
            try {
                \DB::transaction(function () use ($loan, $returnedAt) {
                    ...
                    // Prefer restoring to loan_books.loan_stok when loan_book_id exists
                    if ($loan->loan_book_id) {
                        $loanBook = LoanBook::lockForUpdate()->find($loan->loan_book_id);
                        if ($loanBook) {
                            $previous = $loanBook->loan_stok ?? 0;
                            $loanBook->increment('loan_stok', $qty);
                            StockLog::create([
                                'loan_book_id' => $loanBook->id,
                                'user_id' => auth()->id(),
                                'type' => 'loan',
                                ...
                                'previous_stock' => $previous,
                                'new_stock' => $loanBook->loan_stok,
                                'meta' => json_encode(['loan_id' => $loan->id, 'action' => 'return', 'quantity' => $qty, 'field' => 'loan_stok']),
                            ]);
                        }
                    } elseif ($loan->book_id) {
    ...
    Route::prefix('loan-stock')->name('loan-stock.')->group(function () {
        ...
        Route::get('/{loan_book}/edit', [LoanStockController::class, 'edit'])->name('edit');
        Route::put('/{loan_book}/details', [LoanStockController::class, 'updateDetails'])->name('update.details');
...
Route::middleware(['auth', 'role:owner'])->prefix('owner')->name('owner.')->group(function () {
    ...
    // Owner dapat mengubah status presensi melalui ikon (POST)
...
// Employee auth & dashboard (pegawai)
Route::prefix('employee')->name('employee.')->group(function () {
    Route::get('/login', [\App\Http\Controllers\EmployeeAuthController::class, 'showLogin'])->name('login');
    Route::post('/login', [\App\Http\Controllers\EmployeeAuthController::class, 'login'])->name('login.post');
    Route::post('/logout', [\App\Http\Controllers\EmployeeAuthController::class, 'logout'])->name('logout');

    Route::middleware(['auth:employee'])->group(function () {
        Route::get('/', [\App\Http\Controllers\EmployeeAuthController::class, 'dashboard'])->name('dashboard');
    });
    Route::middleware(['auth:employee'])->group(function () {
        Route::get('/profile', [\App\Http\Controllers\EmployeeAuthController::class, 'profile'])->name('profile');
        Route::patch('/profile', [\App\Http\Controllers\EmployeeAuthController::class, 'update'])->name('profile.update');
        Route::post('/profile/foto', [\App\Http\Controllers\EmployeeAuthController::class, 'uploadFoto'])->name('profile.foto.upload');
    });
    // Employee-specific attendance endpoints (use same controller methods)
    Route::middleware(['auth:employee'])->group(function () {
